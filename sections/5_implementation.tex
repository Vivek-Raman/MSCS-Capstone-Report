...
Form interface that lets users create a new LaTeX project, with options to set the project name, select a template, and configure initial settings, as seen in Figure \ref{fig:ui-newproject}.
...
The agent operates under a comprehensive system prompt that defines its role as an agentic assistant capable of interpreting user requests and making intelligent document modifications.
The prompt establishes guidelines for workflow (list files $\rightarrow$ read $\rightarrow$ edit $\rightarrow$ compile), file organisation conventions, and safety principles.
...
\item The compilation tool is then invoked to check for errors. In case of any errors, the agent repeats the process to resolve them.
...
This includes precise token consumption by measuring both the input and output volume. 
Interactions are tagged with metadata including user ID, model specifications, tool invocations, and session outcomes.
By aggregating these metrics by user, model, and time period, the system provides granular insights into computing expenses and infrastructure scaling.
These data points power real-time dashboards that monitor LLM quality and user feedback.

The dashboard is shown in Figure \ref{fig:posthog-dashboard}, providing a high-level visual overview of the application's usage, performance, and cost, while Figure \ref{fig:posthog-userwise} shows usage per user.

\begin{figure}[htb]
\centering
\includegraphics[width=\linewidth]{figures/posthog-dashboard.png}
\caption{Aggregate usage dashboard in PostHog}
...
\caption{Per-account usage summary in PostHog}
...
\item Compilation errors trigger error feedback instead of silent failures.
\end{itemize}
These constraints guarantee that the agent acts as a trusted scribe rather than an autonomous modifier.
...
In the agent’s case, the compilation status and error message are used to gradually debug and fix the document.

\subsection{User guide}

The user guide is a lightweight static website containing instructions and references to help users get started with Spartan Write.
...
This is accomplished using a reproducible test suite comprising a series of test documents and user requests, along with different scoring metrics.
...
Tests 000, 001, 002, 003, 004, 006, 010, and 011 validate the core ability to create, modify, and reorganise the structure of LaTeX documents.
...
Failures in this category would indicate difficulty understanding parent-child relationships in \LaTeX\ or an inability to preserve document.
These tests assert the basic functionality expected of the agent.

\item \textbf{Search and targeted deletion} \\

Tests 007, 008, and 009 assess how well the language model can find and remove content from long documents, regardless of whether the instructions are crystal clear or a bit vague.
These tests look at two things: how accurately the model can follow instructions when asked to delete a specific item, and how well it can make sense of less direct requests, like removing all mentions of a certain topic, even if the wording changes throughout the document.
...
Tests 005 and 012 check how well the language model really understands an entire document and whether it can write good summaries or add new content that actually fits the context.
...
If a model struggles with these, it usually means it has trouble following longer documents or keeping its new writing in line with what’s already been written.
\item \textbf{Error recovery and problem diagnosis} \\
Tests 014 and 015 look at how well the model can spot and fix broken \LaTeX\ documents, whether the instructions are direct or just hinted at.
Test 014 is simple: it tells the model plainly, "Fix the document."
Test 015 is trickier, expecting the model to realise on its own that something needs repairing, even if the instructions aren’t obvious.
These tests show whether the model can diagnose problems and respond to both clear and subtle hints.
If it fails, that’s a sign it might miss errors, not pick up on what you want, or just not know enough about \LaTeX\ to help.
\item \textbf{Multimodal integration} \\
Tests 013 and 016 see if the model can handle images as part of the document—either by adding image references in the right place or by describing what’s in the image.
...
The dashboard provides a Streamlit-based interactive web interface \cite{streamlit} that loads all results from the benchmark database and organises them by model and test job.
Users can filter results by selected models, execution status, error presence, and keyword search across job IDs and test summaries, as seen in Figure \ref{fig:bench-dashboard}.
The interface renders Plotly charts \cite{plotly} that compare scores across models and display score distributions for deeper analysis.
